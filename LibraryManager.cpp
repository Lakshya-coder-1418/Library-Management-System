...
void LibraryManager::searchBook() const {
    string isbn;
...
void LibraryManager::addUser() {
    ...
    if (name.empty() || id.empty()) {
        cout << "User name and ID cannot be empty.\n";
        return;
    }
    if (findUser(id)) {
...
const Book* LibraryManager::findBookByISBN(const string &isbn) const {
    for (auto &book : books) {
        if (book.getISBN() == isbn) {
            return &book;
        }
